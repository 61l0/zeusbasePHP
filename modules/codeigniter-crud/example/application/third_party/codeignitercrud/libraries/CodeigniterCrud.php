...
require_once dirname(dirname ( __FILE__ )) . '/include/FileUpload.php';
require_once dirname(dirname ( __FILE__ )) . '/include/Image.php';
require_once dirname(dirname ( __FILE__ )) . '/include/html.class.php';
class CodeigniterCrud {
	private $table;
	private $tableAlias;
	...
	public function __construct($conf = array()) {
		if (isset($conf['actionKey'])){
			$this->actionKey = $conf['actionKey'];
		}
		if (isset($conf['pageKey'])){
			$this->pageKey = $conf['pageKey'];
		}
		if (isset($conf['language'])){
			$this->language = $conf['language'];
		}
		if (isset($conf['mysqlcharset'])){
			$this->mysqlcharset = $conf['mysqlcharset'];
		}
		...
		$this->CI->db->query("SET CHARACTER SET " . $this->mysqlcharset );
		
		$lang = 'en';
		if (! empty ( $this->language )) {
			$lang = $this->language;
		}
		if (file_exists ( dirname(dirname ( __FILE__ )) . '/language/lang.' . $lang . '.php' )) {
			require_once dirname(dirname ( __FILE__ )) . '/language/lang.' . $lang . '.php';
		} else {
			require_once dirname(dirname ( __FILE__ )) . '/language/lang.en.php';
		}
		$this->themePath = dirname(dirname ( __FILE__ )).'/theme';
		$this->theme = 'bootstrap';
	}
	public function table($table) {
		$this->uid = sha1 ( $table );
		if (! $this->CI->session->userdata($this->uid)) {
			$this->CI->session->set_userdata($this->uid,array());
		}
	...
	private function buildCondition() {
		$session = $this->CI->session->userdata($this->uid);
		if (! isset ( $_POST ['search'] ) && isset ( $session['search'] )) {
			$_POST ['search'] = $session['search'];
		}

		if (isset ( $_POST ['search'] )) {
			$session['search'] = $_POST ['search'];
			$this->CI->session->set_userdata($this->uid,$session);
		}
	...
	private function index() {
		...
		if (file_exists($this->themePath.'/'.$this->theme.'/index.php')){
			...
			$template = new IndexTemplate ( $this->CI );
	...
	private function form() {
		...
		if (! empty ( $_POST ) && $_POST ['token'] == $session[$_POST ['token_key']] && $this->uid . "_auth_token" . '_' . md5 ( $tokenId ) == $_POST ['token_key']) {
				...
				case 'confirm' :
					...
					if (! empty ( $this->validates )) {
						if (! class_exists ( 'AbstractValidator' )) {
							require dirname(dirname ( __FILE__ )) . '/validation/AbstractValidator.php';
						}
						if (! class_exists ( 'FormValidator' )) {
							require dirname(dirname ( __FILE__ )) . '/validation/FormValidator.php';
						}
						...
						foreach ( $this->validates as $field => $args ) {
							...
							if (! class_exists ( $class )) {
								require dirname(dirname ( __FILE__ )) . '/validation/' . $class . '.php';
							}
	...
	private function delete() {
		...
		if (! empty ( $_POST )) {
			...
			if ($_POST ['token'] == $session[$_POST ['token_key']] && $this->uid . "_auth_token" . '_' . md5 ( $tokenId ) == $_POST ['token_key']) {
				switch ($_POST ['action_type']) {
					case 'delete' :
	...
	public function insert($req, $table) {
		if (! empty ( $req [$table] )) {
			...
			$this->CI->db->query($sql,$v);
			$this->insetId = $this->CI->db->insert_id();
		}
	}
			...
			if (count ( $tmp ) == 3 && $this->uid == $tmp [0]) {
				...
				if (! empty ( $rs )) {
					...
					if (! empty ( $this->primaryKeys )) {
						foreach ( $this->primaryKeys as $k3 => $v3 ) {
							foreach ( $v3 as $v4 ) {
								if ($k3 == $this->table && trim ( $_GET ['key'] [$k3 . '_' . $v4] ) != '') {
									$_POST [$tmp [0]] [$k3] [$v4] = $_GET ['key'] [$k3 . '_' . $v4];
								}
							}
						}
					}
					...
					if (! empty ( $type ['path'] ) && file_exists ( $type ['path'] . '/' . $rs [$tmp [0]] [$tmp [1]] [$tmp [2]] )) {
						...
						$query = $this->CI->db->query($sql,$vl);
						$rsOther = $query->result_array();
	...
	private function debug($sql = null,$params = array()) {
		if (MysqliCrudConfig::$sqlDebug == true){
